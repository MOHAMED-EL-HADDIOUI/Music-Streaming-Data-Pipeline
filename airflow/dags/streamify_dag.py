...
BIGQUERY_DATASET = os.environ.get('BIGQUERY_DATASET', 'music-streaming-data-pipeline-stg')
...
with DAG(
    dag_id = f'music-streaming-data-pipeline-dag',
    default_args = default_args,
    description = f'Hourly data pipeline to generate dims and facts for music streaming data pipeline',
    schedule_interval="5 * * * *", #At the 5th minute of every hour
    start_date=datetime(2022,3,26,18),
    catchup=False,
    max_active_runs=1,
    user_defined_macros=MACRO_VARS,
    tags=['music-streaming-data-pipeline']
) as dag:
    ...
    execute_dbt_task = BashOperator(
        task_id = 'dbt_music_streaming_data_pipeline_run',
