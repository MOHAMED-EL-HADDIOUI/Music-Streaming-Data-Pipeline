...
resource "google_compute_instance" "kafka_vm_instance" {
  name                      = "music-streaming-data-pipeline-kafka-instance"
...
resource "google_compute_instance" "airflow_vm_instance" {
  name                      = "music-streaming-data-pipeline-airflow-instance"
  machine_type              = "e2-standard-4"
  allow_stopping_for_update = true
...
resource "google_dataproc_cluster" "mulitnode_spark_cluster" {
  name   = "music-streaming-data-pipeline-multinode-spark-cluster"
  region = var.region
